...
const double goalWeight = 0.2;
const double obsWeight = 1.2;
const double speedWeight = 0.0;
...
bool MPCProblem::operator()(const T* const v, const T* const w, T* residual) const {
    ...
    for (int i = 0; i < predictionHorizon; ++i) {
        ...
        //      T dist = T (sqrt((x - obs_list[i][0])*(x - obs_list[i][0]) + (y - obs_list[i][1])*(y -
        //      obs_list[i][1]))) ; obstacle_residual += ceres::abs(T(safetyRadius) - dist);
        //  }
...
void MPCController::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    ceres::CostFunction* cost_function
      = new ceres::AutoDiffCostFunction<MPCProblem, 4, 1, 1>(new MPCProblem(x, y, theta, x_goal_, y_goal_, obs_list));
    ...
    if (std::abs(v) < robot_stuck_flag_cons || std::abs(msg->twist.twist.linear.x) < robot_stuck_flag_cons)
